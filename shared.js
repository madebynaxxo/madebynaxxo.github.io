...
(function(){
  ...
  if(!ham || !links) return;
  function close(){ ham.classList.remove('open'); links.classList.remove('open'); document.body.style.overflow=''; }
...
document.querySelectorAll('.rtable').forEach(t => {
  const w = document.createElement('div');
  w.className = 'rtable-scroll';
...
(function(){
  ...
  lb.addEventListener('touchend', e=>{
    ...
    if(Math.abs(dx) > 50){
      ...
      if(dx < 0 && next) next.click(); else if(prev) prev.click();
    }
  });
})();
